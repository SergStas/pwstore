...
class SpellHandler:
    __no_arg = {
        SpellEvent.first_launch:
            'Приветствую на самой честной и бесплтаной площадке по продаже персонадей в Perfect World!\n'
        ...
        SpellEvent.help:
            ...
            '```/help``` - помощь;\n'
            '```/find_account``` - перейти к поиску аккаунтов\n'
            '```/sell_account``` - продажа аккаунта или управление лотами выставленных вами аккаунтов'
    }
    __with_args = {
    ...
    def get_message(event: SpellEvent, args=None) -> Optional[str]:
        ...
        return SpellHandler.__no_arg[event].format(*args)
